...
namespace Net.Data.Sap
{
    public class TakeInventoryFinishedProductsRepository : RepositoryBase<TakeInventoryFinishedProductsEntity>, ITakeInventoryFinishedProductsRepository
    {
        ...
        public TakeInventoryFinishedProductsRepository(IConnectionSQL context, IConfiguration configuration, DataContextSeg dbSeg, DataContextSap dbSap, CompanyProviderSap companyProviderSap)
            : base(context)
        {
            _dbSeg = dbSeg;
            _dbSap = dbSap;
        ...
        public async Task<ResultadoTransaccionEntity<SolicitudTraslado1QueryEntity>> GetToCopy(TakeInventoryFinishedProductsToCopyFindEntity value)
        {
            var resultTransaccion = new ResultadoTransaccionEntity<SolicitudTraslado1QueryEntity>
            {
                NombreMetodo = regex.Match(System.Reflection.MethodBase.GetCurrentMethod().DeclaringType.Name).Groups[1].Value,
                NombreAplicacion = _aplicacionName
            };
            ...
            try
            {
                ...
                var tipoOperacion = await _dbSap.TipoOperacion.Where(t => t.Code == "11").FirstOrDefaultAsync();
                ...
                .Select(x => new SolicitudTraslado1QueryEntity
                {
                    ItemCode = x.Key.ItemCode,
                    Dscription = x.Key.Dscription,
                    ...
                    UnitMsr = x.Key.UnitMsr,
                    Quantity = (x.Key.UnitMsr ?? "").ToUpper() == "KG" ? x.SumKg : x.SumQty,
                    OpenQty = (x.Key.UnitMsr ?? "").ToUpper() == "KG" ? x.SumKg : x.SumQty,
                    U_FIB_OpQtyPkg = x.SumKg
                })
                .OrderBy(x => x.ItemCode)
                .ToListAsync();
