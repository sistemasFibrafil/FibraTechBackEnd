...
namespace Net.Data.SAPBusinessOne
{
    public class ItemsRepository : RepositoryBase<ItemsEntity>, IItemsRepository
    {
        ...
        public ItemsRepository(IConnectionSQL context, IConfiguration configuration, DataContextSAPBusinessOne db, CompanyProviderSAPBusinessOne companyProviderSap)
            : base(context)
        {
            _db = db;
        ...
        public async Task<ResultadoTransaccionEntity<ItemsQueryEntity>> GetListByCode(ItemsFindByListCodeEntity value)
        {
            ...
            try
            {
                ...
                {
                    // 🔹 Obtiene datos del socio de negocio
                    businessPartners = await _db.BusinessPartners
                    .Where(n => n.CardCode == value.CardCode)
                    .Select(n => new BusinessPartnersEntity
                    {
                        ListNum = n.ListNum,
                        ShipToDef = n.ShipToDef
                ...
                .Select(i => new
                {
                    Item = i,
                    Price = i.PriceLists
                            .Where(p => p.PriceList == businessPartners.ListNum && p.Currency == value.Currency)
                ...
                .Select(x => new ItemsQueryEntity
                {
                    ...
                    OnHand = _db.ItemWarehouseInfo
                             .Where(w => w.ItemCode == x.Item.ItemCode && warehouse.Contains(w.WhsCode))
        ...
        public async Task<ResultadoTransaccionEntity<ItemsEntity>> SetCreateMassive(ItemsCreateMassiveEntity value)
        {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    if(value.IsSalida)
                    {
                        var listaItemsOrigen = value.Lines
                    ...
                    .ToList();
                        ...
                        foreach (var articulo in listaItemsOrigen)
                        {
                            ...
                            salida.Lines.Quantity = Convert.ToDouble(articulo.OnHand);
                            salida.Lines.Add();
                        }
                   ...
                   .ToList();
                        ...
                        {
                            var lista = value.Lines.Where(x => x.U_FIB_ItemCode == articuloGroup.U_FIB_ItemCode).ToList();
                            ...
                            foreach (var articulo in lista)
                            {
                                ...
                                entrada.Lines.Quantity = Convert.ToDouble(articulo.OnHand);
                                entrada.Lines.Add();
                            }
