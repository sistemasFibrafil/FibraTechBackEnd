...
namespace Net.Data.SAPBusinessOne
{
    public class InventoryTransferRequestRepository : RepositoryBase<InventoryTransferRequestEntity>, IInventoryTransferRequestRepository
    {
        ...
        public InventoryTransferRequestRepository(IConnectionSQL context, IConfiguration configuration, DataContextSAPBusinessOne db, CompanyProviderSAPBusinessOne companyProviderSap)
            : base(context)
        {
            _db = db;
        ...
        public async Task<ResultadoTransaccionEntity<InventoryTransferRequestOpenQueryEntity>> GetListOpen()
        {
            ...
            try
            {
                var list = await _db.InventoryTransferRequest
                .Where(n => n.DocStatus == "O" && n.U_FIB_DocStPkg == "O" && n.U_FIB_IsPkg == "Y")
                .Select(s => new InventoryTransferRequestOpenQueryEntity
                {
                    DocEntry = s.DocEntry,
                    DocNum = s.DocNum,
                    WhsCode = s.Filler
        ...
        public async Task<ResultadoTransaccionEntity<InventoryTransferRequestEntity>> GetListByFilter(InventoryTransferRequestFilterEntity value)
        {
            ...
            try
            {
                var query = _db.InventoryTransferRequest
                .AsNoTracking()
                ...
                .Select(n => new InventoryTransferRequestEntity
                {
                    DocEntry = n.DocEntry,
                    DocNum = n.DocNum,
        ...
        public async Task<ResultadoTransaccionEntity<InventoryTransferRequestQueryEntity>> GetByDocEntry(int docEntry)
        {
            ...
            try
            {
                var data = await _db.InventoryTransferRequest
                .Where(n => n.DocEntry == docEntry)
                .Select(n => new InventoryTransferRequestQueryEntity
                {
                    DocEntry = n.DocEntry,
                    DocNum = n.DocNum,
                    DocStatus = n.DocStatus,
        ...
        /// <summary>
        /// Se obtiene la solicitud de traslado para ser convertida en traslado de stock
        /// </summary>
        ...
        public async Task<ResultadoTransaccionEntity<InventoryTransferRequestQueryEntity>> GetToTransferenciaByDocEntry(int docEntry)
        {
            ...
            try
            {
                ...
                .Select(s => new InventoryTransferRequestQueryEntity
                {
                    CardCode = s.CardCode,
                    CardName = s.CardName,
                ...
                .Select(s => new InventoryTransferRequest1QueryEntity
                {
                    ...
                    WhsCode = s.WhsCode,
                    U_tipoOpT12 = s.U_tipoOpT12 ?? "",
                    U_tipoOpT12Nam = s.OperationType.U_descrp ?? "",
                    UnitMsr = s.UnitMsr,
                    Quantity = s.Quantity,
                    OpenQty = s.OpenQty,
                    U_FIB_OpQtyPkg = s.U_FIB_OpQtyPkg ?? 0,
        ...
        public async Task<ResultadoTransaccionEntity<InventoryTransferRequestEntity>> SetCreate(InventoryTransferRequestCreateEntity value)
        {
            ...
            StockTransfer inventoryTransferRequest = null;
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    // Se crea el objeto de la spolicitud de traslado
                    inventoryTransferRequest = company.GetBusinessObject(BoObjectTypes.oInventoryTransferRequest);


                    #region <<< CABECERA >>>

                    inventoryTransferRequest.DocDate = value.DocDate;
                    inventoryTransferRequest.DueDate = value.DocDueDate;
                    inventoryTransferRequest.TaxDate = value.TaxDate;
                    inventoryTransferRequest.DocObjectCode = BoObjectTypes.oInventoryTransferRequest;
                    
                    inventoryTransferRequest.UserFields.Fields.Item("U_FIB_IsPkg").Value = value.U_FIB_IsPkg;
                    inventoryTransferRequest.UserFields.Fields.Item("U_FIB_DocStPkg").Value = value.U_FIB_DocStPkg;

                    // ===========================================================================================
                    // SOCIO DE NEGOCIO
                    // ===========================================================================================
                    inventoryTransferRequest.CardCode = value.CardCode;
                    inventoryTransferRequest.CardName = value.CardName;
                    inventoryTransferRequest.ContactPerson = value.CntctCode;
                    inventoryTransferRequest.Address = value.Address;

                    // ===========================================================================================
                    // ALMACEN
                    // ===========================================================================================
                    inventoryTransferRequest.FromWarehouse = value.Filler;
                    inventoryTransferRequest.ToWarehouse = value.ToWhsCode;

                    // ===========================================================================================
                    // OTROS
                    // ===========================================================================================
                    inventoryTransferRequest.UserFields.Fields.Item("U_FIB_TIP_TRAS").Value = value.U_FIB_TIP_TRAS;
                    inventoryTransferRequest.UserFields.Fields.Item("U_BPP_MDMT").Value = value.U_BPP_MDMT;
                    inventoryTransferRequest.UserFields.Fields.Item("U_BPP_MDTS").Value = value.U_BPP_MDTS;

                    // ===========================================================================================
                    // PIE
                    // ===========================================================================================
                    inventoryTransferRequest.SalesPersonCode = value.SlpCode;
                    inventoryTransferRequest.JournalMemo = value.JrnlMemo;
                    inventoryTransferRequest.Comments = value.Comments;

                    // ===========================================================================================
                    // AUDITORIA
                    // ===========================================================================================
                    inventoryTransferRequest.UserFields.Fields.Item("U_UsrCreate").Value = value.U_UsrCreate;
                    ...
                    {
                        inventoryTransferRequest.Lines.ItemCode = line.ItemCode;
                        inventoryTransferRequest.Lines.ItemDescription = line.Dscription;
                        inventoryTransferRequest.Lines.FromWarehouseCode = line.FromWhsCod;
                        inventoryTransferRequest.Lines.WarehouseCode = line.WhsCode;
                        inventoryTransferRequest.Lines.Quantity = line.Quantity;

                        inventoryTransferRequest.Lines.UserFields.Fields.Item("U_tipoOpT12").Value = line.U_tipoOpT12;
                        inventoryTransferRequest.Lines.UserFields.Fields.Item("U_FIB_OpQtyPkg").Value = line.U_FIB_OpQtyPkg;
                        inventoryTransferRequest.Lines.UserFields.Fields.Item("U_FIB_LinStPkg").Value = line.U_FIB_LinStPkg;
                        inventoryTransferRequest.Lines.Add();
                    }

                    #endregion


                    var reg = inventoryTransferRequest.Add();

                    if (reg == 0)
                    {
                        #region <<< PICKING >>>

                        // Se verifica si el picking tiene valores para ser registrado, de lo contrario se omite la creación del mismo
                        if (value.PickingLines.Count > 0)
                        {
                            ...
                            // Se crea el objeto de la solicitud de traslado
                            inventoryTransferRequest = company.GetBusinessObject(BoObjectTypes.oInventoryTransferRequest);


                            // Se obtiene la solicitud de traslado creada para generar el picking
                            if (!inventoryTransferRequest.GetByKey(docEntry)) throw new Exception("No existe la solictud de traslado.");
                            ...
                            foreach (var line in value.PickingLines)
                            {
                                for (int i = 0; i < inventoryTransferRequest.Lines.Count; i++)
                                {
                                    ...
                                    if (inventoryTransferRequest.Lines.ItemCode == line.U_ItemCode && inventoryTransferRequest.Lines.FromWarehouseCode == line.U_FromWhsCod)
                                    {
                                        oGeneralData.SetProperty("U_BaseEntry", inventoryTransferRequest.DocEntry);
                                        oGeneralData.SetProperty("U_BaseNum", inventoryTransferRequest.DocNum);
                                        oGeneralData.SetProperty("U_BaseType", inventoryTransferRequest.DocObjectCode);
                                        oGeneralData.SetProperty("U_BaseLine", inventoryTransferRequest.Lines.LineNum);
                                        oGeneralData.SetProperty("U_DocDate", inventoryTransferRequest.DocDate);
                                        oGeneralData.SetProperty("U_TaxDate", inventoryTransferRequest.TaxDate);
                                        oGeneralData.SetProperty("U_DocDueDate", inventoryTransferRequest.DueDate);

                                        if (!string.IsNullOrWhiteSpace(inventoryTransferRequest.CardCode)) oGeneralData.SetProperty("U_CardCode", inventoryTransferRequest.CardCode);
                                        if (!string.IsNullOrWhiteSpace(inventoryTransferRequest.CardName)) oGeneralData.SetProperty("U_CardName", inventoryTransferRequest.CardName);
                                        if (!string.IsNullOrWhiteSpace(inventoryTransferRequest.Lines.ItemCode)) oGeneralData.SetProperty("U_ItemCode", inventoryTransferRequest.Lines.ItemCode);
                                        if (!string.IsNullOrWhiteSpace(inventoryTransferRequest.Lines.ItemDescription)) oGeneralData.SetProperty("U_Dscription", inventoryTransferRequest.Lines.ItemDescription);
                                        if (!string.IsNullOrWhiteSpace(line.U_CodeBar)) oGeneralData.SetProperty("U_CodeBar", line.U_CodeBar);
                                        if (!string.IsNullOrWhiteSpace(line.U_FromWhsCod)) oGeneralData.SetProperty("U_FromWhsCod", line.U_FromWhsCod);
                                        if (!string.IsNullOrWhiteSpace(line.U_WhsCode)) oGeneralData.SetProperty("U_WhsCode", line.U_WhsCode);

                                        oGeneralData.SetProperty("U_UnitMsr", line.U_UnitMsr);
                                        oGeneralData.SetProperty("U_Quantity", Convert.ToDouble(Math.Round(line.U_Quantity, 6)));
                                        oGeneralData.SetProperty("U_WeightKg", Convert.ToDouble(Math.Round(line.U_WeightKg, 6)));
                    ...
                    resultTransaccion.IdRegistro = 0;
                    resultTransaccion.ResultadoCodigo = 0;
                    resultTransaccion.ResultadoDescripcion = "La solictud de traslado registrada con éxito.";
                }
                catch (Exception ex)
                {
                    resultTransaccion.IdRegistro = -1;
                ...
                {
                    _companyProviderSap.LiberarObjetosCOM(inventoryTransferRequest, oGeneralData, oCompService, oGeneralService);
                }
        ...
        public async Task<ResultadoTransaccionEntity<InventoryTransferRequestEntity>> SetUpdate(InventoryTransferRequestUpdateEntity value)
        {
            ...
            return await Task.Run(() =>
            {
                try
                {
                    ...
                    stockTransfer.UserFields.Fields.Item("U_FIB_IsPkg").Value = value.U_FIB_IsPkg;
                    ...
                    foreach (var line in value.Lines.Where(x => x.Record == 1))
                    {
                        ...
                        stockTransfer.Lines.UserFields.Fields.Item("U_FIB_OpQtyPkg").Value = line.U_FIB_OpQtyPkg;
                    }
                    ...
                    if (reg == 0)
                    {
                        resultTransaccion.IdRegistro = 0;
                        resultTransaccion.ResultadoCodigo = 0;
                        resultTransaccion.ResultadoDescripcion = "La solictud de traslado actualizada con éxito.";
                    }
                    else
                    {
                        company.GetLastError(out int errorCode, out string errorMessage);
                ...
                try
                {
                    ...
                    if (regUpdate == 0)
                    {
                        ...
                        if (regClose == 0)
                        {
                            resultTransaccion.IdRegistro = 0;
                            resultTransaccion.ResultadoCodigo = 0;
                            resultTransaccion.ResultadoDescripcion = "La solictud de traslado cerrada con éxito ..!";
                        }
                        else
                        {
                            company.GetLastError(out int errorCode, out string errorMessage);
            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(_cnxSap))
                {
                    ...
                    using (SqlCommand cmd = new SqlCommand(SP_GET_SOLICITUDTRASLADO_BY_DOCENTRY, conn))
                    {
                        ...
                        {
                            header = context.Convert<InventoryTransferRequestPrintEntity>(reader);
                        }
                    }
                    ...
                    using (SqlCommand cmd = new SqlCommand(SP_GET_LIST_SOLICITUDTRASLADO_DETALLE_BY_DOCENTRY, conn))
                    {
                        ...
                        using (var reader = await cmd.ExecuteReaderAsync())
                        {
                            linea = (List<InventoryTransferRequest1PrintEntity>)context.ConvertTo<InventoryTransferRequest1PrintEntity>(reader);
                        }
                    }
