...
namespace Net.Business.Services.Controllers.SAPBusinessOne.Administration.Definitions.General
{
    ...
    public class ProcesoController : ControllerBase
    {
        ...
        {
            var objectGetList = await _repository.Processes.GetList();
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<IActionResult> GetListByFiltro([FromQuery] ProcesoFindRequestDto value)
        {
            var objectGetList = await _repository.Processes.GetListByFiltro(value.ReturnValue());
